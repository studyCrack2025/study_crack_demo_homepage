...
export function GuidePreview({ step, profile, aquarium, catalog, tasks = [] }) {
  ...
  if (step === 3) return <div className="product-guide-scene"><AquariumScene variant="guide" slots={aquarium?.slots} catalog={catalog} /><span>{aquarium?.activeCount ? '공부 보상을 확인한 뒤 먹이로 돌봐주세요.' : '물고기를 수조에 배치하면 여기에 보여요.'}</span></div>;
